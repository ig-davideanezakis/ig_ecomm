...
export async function GET(request: NextRequest) {
  ...
  if (category) {
    idx++; conditions.push(`c.slug = $${idx}`);
    queryParams.push(category);
  }
  if (brand) {
    idx++; conditions.push(`b.slug = $${idx}`);
  ...
  const countResult = await pool.query(
    `SELECT COUNT(*)::int as total FROM "product" p
     ...
     LEFT JOIN "brand" b ON p."brand_id" = b.id ${whereClause}`,
    queryParams,
  );
...
export async function POST(request: Request) {
  ...
  const { identifier, title, description, content, basePrice, compareAtPrice, costPrice,
  ...
  const ident = identifier || `PROD-${Date.now().toString(36).toUpperCase()}`;
  ...
  const result = await pool.query(
    `INSERT INTO "product" (identifier, title, slug, description, content,
     ...
     RETURNING id`,
    [ident, title, slug, description || null, content || null,
